...
{
	int k;
	STRING synfind(),getident(),getword2(),getstring(),getword();
	float getnum();
...
{  int i,c;
    ...
    if(c=='"'){ for(s=(char *)buffer; (c=lexgetc())!='"'; *s++ = c ){
		...
		*s = '\0';
        ...
        if((lexin=fopen((char *)buffer,"r"))==NULL){
		...
		in_files[in_index].in_name = strsave((char *)buffer);
...
{       int l;
	...
	*p = '\0';
	if (l >8 ) p[8] = '\0';
	return((STRING) strsave((char *)buffer));
}
